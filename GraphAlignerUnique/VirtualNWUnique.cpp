...
#include "../GraphAligner/VirtualNW.h"
...
void NWEdge::checkConsistency()
{
	...
	if(path != 0)
	{
		...
		if(path->table != 0)
		{
			if(! (to_y <= (int)path->table->S->length()))
			{
				std::cerr << "to_y: "<< to_y << "\n";
				std::cerr << "path->table->S->length(): "<< path->table->S->length() << "\n" << std::flush;
			}
			...
			if(usedGraphEdge != 0)
			{
				...
				if(! (path->table->gA->nodesPerLevel_ordered.at(to_x).at(to_z)->Incoming_Edges.count(usedGraphEdge) > 0))
				{
...
void NWPath::takeInExtensionPath(localExtension_pathDescription* pD, int entryExit, int completeToEnd)
{
	...
	else
	{
		...
		if(completeToEnd == 1)
		{
			...
			int lastYCoordinate = table->S->length();
			int missingYs = table->S->length() - lastCoordinates.at(1);
			assert(missingYs >= 0);
