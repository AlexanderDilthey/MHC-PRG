...
std::vector< std::pair<seedAndExtend_return_local, seedAndExtend_return_local> > GraphAlignerUnique::seedAndExtend_short_allAlignments(oneReadPair readPair, double insertSize_mean, double insertSize_sd, bool greedyLocalExtension, bool debug, bool MiSeq250bp)
{
	...
	if(verbose)
	{
		std::cout << "GraphAlignerUnique::seedAndExtend_short_allAlignments(..): Post-filtering read1_backtraces.size(): " << read1_backtraces.size() << "\n";
	...
	if(verbose) std::cout << "GraphAlignerUnique::seedAndExtend_short_allAlignments(..):  Read 2 alternatives:\n" << std::flush;
	...
	{
		double q = combinedScores_nonGenomic.at(i);
		forReturn.at(i).first.mapQ = q;
		forReturn.at(i).second.mapQ = q;

		forReturn.at(i).first.mapQ_genomic = q;
	...
	for(unsigned int i = 0; i < forReturn.size(); i++)
	{
		...
		std::string r1_reverse = (forReturn.at(i).first.reverse) ? "minus" : "plus";
		std::string r2_reverse = (forReturn.at(i).second.reverse) ? "minus" : "plus";
	...
	for(unsigned int i = 0; i < forReturn.size(); i++)
	{
		...
		for(unsigned int j = 0; j < forReturn.at(i).first.graph_aligned.length(); j++)
		{
			...
			if(!((Q - 1) <= 1e-5))
			{
				...
				std::cerr << "positionID: " << positionID << "\n";
				std::cerr << "j: " << j << "\n";
				std::cerr << "forReturn.at(i).first.graph_aligned.length(): " << forReturn.at(i).first.graph_aligned.length() << "\n";
				std::cerr << std::flush;
			}
			assert((Q - 1) <= 1e-5);
			if(Q > 1)
			...
			assert(Q >= forReturn.at(i).first.mapQ_genomic);
		...
		for(unsigned int j = 0; j < forReturn.at(i).second.graph_aligned.length(); j++)
		{
			...
			if(!((Q - 1) <= 1e-5))
			{
				...
				std::cerr << "positionID: " << positionID << "\n";
				std::cerr << "j: " << j << "\n";
				std::cerr << "forReturn.at(i).second.graph_aligned.length(): " << forReturn.at(i).second.graph_aligned.length() << "\n";
				std::cerr << std::flush;
			}
			assert((Q - 1) <= 1e-5);
			if(Q > 1)
			...
			assert(Q >= forReturn.at(i).second.mapQ_genomic);			
...
std::pair<seedAndExtend_return_local, seedAndExtend_return_local> GraphAlignerUnique::seedAndExtend_local_paired_or_short(oneReadPair readPair, bool usePairing, bool use_short, double insertSize_mean, double insertSize_sd, bool estimateInsertSize, std::map<int, double>& insertSize_posterior_ret, bool MiSeq250bp)
{
	...
	// this section here can be filled with output from extractReadsAlignment.pl
	// there are two of those - here and in seedAndExtend_short_allAlignments
...
seedAndExtend_return_local GraphAlignerUnique::seedAndExtend_short(std::string sequence_nonReverse, std::vector<seedAndExtend_return_local>& allBacktraces, bool greedyLocalExtension, bool MiSeq250bp)
{
	...
	if(chains_for_sequence.size() > 0)
	{
		...
		std::sort(allBacktraces_preLimit.begin(), allBacktraces_preLimit.end(), [](const seedAndExtend_return_local& a, const seedAndExtend_return_local& b){
			return (a.Score < b.Score);
		});

		std::reverse(allBacktraces_preLimit.begin(), allBacktraces_preLimit.end());
		if(allBacktraces_preLimit.size() > 1)
		{
			assert(allBacktraces_preLimit.at(0).Score >= allBacktraces_preLimit.at(1).Score);
		}
	...
	{
		seedAndExtend_return_local selectedBacktrace_forReturn;
		selectedBacktrace_forReturn.graph_aligned.resize(sequence.length(), '_');
		selectedBacktrace_forReturn.graph_aligned_levels.resize(sequence.length(), -1);
