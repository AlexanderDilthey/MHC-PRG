...
public:
	...
	VirtualNWTable(GraphAligner* aligner, std::string* sequence) : gA(aligner), S(sequence)
	{
...
public:
	...
	void verify_edges();
	void createAndAddEdge(int from_x, int from_y, int from_z, int to_x, int to_y, int to_z, Edge* graphEdge, int firstOrLast = 0, int entryExitStatus = 0);
	...
	void takeInExtensionPath(localExtension_pathDescription* pD, int entryExit);
